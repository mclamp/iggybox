...
static GdkColor *get_hash_color(Sequence *seq,int pos,Viewport *viewport) {
  ...
  double av = 1.0*dbsize/(1.0*nmer2pow(m));
...
int pixel_to_position(int n, int m,int *seqnum, int *seqpos) {

  int offset          = (int)vadjust->value;

  int h               = n/viewport->char_height;

  int chars_per_row   = viewport->width/viewport->char_width;
  
  int rows_per_chunk  = align_height + 2;

  int chunknum        = (int)((offset + h)/rows_per_chunk);

  printf("Offset %d Pos %d chars/row %d rows/ch %d chunk %d\n",offset,h,chars_per_row,rows_per_chunk,chunknum);

  *seqnum   = (int)(offset + h) % rows_per_chunk;
  *seqpos   = (int)chunknum*chars_per_row +  m/viewport->char_width;
...
void button_press_event(GtkWidget *widget,
  ...
  pixel_to_position(event->y,event->x,&seqnum,&charnum);
  
  seq = Sequence_get_Sequence_at(head,seqnum,align_height);

  printf("Seqnum/pos %d %d\n",seqnum,charnum);
		   ...
		   GtkFileSelection *fs) 
{
  ...
  vadjust->page_size = viewport->height/viewport->char_height;
  //vadjust->upper = align_height;
  vadjust->page_increment = vadjust->page_size/2;
  
  hadjust->page_size = viewport->width/viewport->char_width;
  hadjust->upper = Sequence_max_sequence_length(head);
  hadjust->page_increment = hadjust->page_size/2;
...
static void  zoom_in()
{
  ...
  //  GTK_ADJUSTMENT(vadjust)->upper = (gdouble)(1+Sequence_max_sequence_length(head)/(align_area->allocation.width/viewport->char_width) * (2 + align_height));
  // GTK_ADJUSTMENT(vadjust)->upper = (gdouble)(1+Sequence_max_sequence_length(head)/(align_area->allocation.width/viewport->char_width) * (2 + align_height));
